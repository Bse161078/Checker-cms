...
const showToast = (message) => {
  ...
  const filteredLevels = Level&&Level.filter(level => {
    const filteredRooms = level.rooms.filter(room => {
      return room.name && room.name.toLowerCase().includes(search.toLowerCase());
    });
                  ...
                  <div class="flex flex-wrap -mx-1">
                    {el?.rooms?.length>0?el?.rooms.map((room, ind) => {
                      return (
                        <div
                          ...
                          onClick={() => {
                            ...
                            if (room.cleaning_status === "IN_PROGRESS") {
                              ...
                              showToast('Room is already in progress! Please wait for checker response');
                        ...
                        >
                          <div class="bgCard p-4" style={{cursor:'pointer'}}>
